...
The following Wells were excluded from Media and DMSO controls: NIL. <br>
...
norm_CTG <- annotationsCTG %>%
  ...
  filter(!Well_ID %in% c("O13","I13","H13","G13")) %>% 
  group_by(Patient) %>% 
...
By randomising/scrambling the category labels, we create a data set that should not be predicted correctly and indeed, the prediction accuracy for the scrambled data drops down to 53%. 
...
**Here, changing cost from 1 to 50 and keep gamma at default improved the model performance from 83% to 99% accuracy.**
...
Testing the SVM model on the last 20% of data gives a 72% accuracy in prediction.  
