---
title: "<img src=\"Cropped_VCFG_Logo2019.png\" style=\"float:right;height:77.1429px;width:152px\"/>ORG66BR3<br>3D Image-based Classification <br> SVM analyses<br>"
output: 
  html_document:
    keep_md: true
    theme: flatly
...
*in addition to Staurosporine and Mitomycin, we added  Doxorubicin to the positive controls.*
...
annotationsCTG <- read_csv(here::here("data/OC_ORG66BR3_CTG.csv")) %>% dplyr::select(-WELL_ID)
...
```{r}
annotationsCTG <- read_csv(here::here("data/OC_ORG66BR3_CTG.csv")) 
...
*Need to update with images for ORG66BR3*
```{r, echo=FALSE}
...
**Here, changing cost from 1 to 5 and changing gamma from a default of 0.01 to 0.03 improved the model performance from 83% to 98% accuracy.**
...
Testing the SVM model on the last 20% of data gives a 75% accuracy in prediction.  
...
Testing the SVM model on the last 20% of data after scrambling the category labels gives a random 55% accuracy in prediction.  
...
write.csv(summary, here::here(paste0("output/",patient,"/summary_morphology_ORG66BR3.csv")))
```
...
write.csv(pred_all_data, here::here(paste0("output/", patient,"/pred_all_data_ORG66BR3.csv")))
...
normplot <- data_norm_plot %>%
  ...
  labs(title = "SVM results ORG66BR3\n",
