---
title: "<img src=\"Cropped_VCFG_Logo2019.png\" style=\"float:right;height:77.1429px;width:152px\"/>SMALL MIXED DENSE ORGANOIDS<br>3D Image-based Classification <br> SVM analyses<br>"
output: 
  html_document:
    keep_md: true
    theme: flatly
...
* Alive/ healthy control (Vehicle control Wells from SMALL_MIXED_DENSE organoids)  
...
```{r}
prefix <- "SMALL_MIXED_DENSE"
patient <- "SMALL_MIXED_DENSE"
```
...
Raw single organoid data was imported from CellProfiler outputs and CTG with annotations was imported from previous R outputs. Wells were excluded on a per-orgaonid QC basis, and CTG normalisation to DMSO/Media controls was completed within and organoid, before normalised results were combined together for all SMALL_MIXED_DENSE organoids,
 <br>


```{r}
### Define organoids of interest
### ORG61BR2 MISSINF AREASHAPE_CONVEXARE
small_mixed_dense_organoids <- c('ORG38BR2','ORG41BR1','ORG41BR2','ORG46BR1','ORG49BR2','ORG55BR1','ORG55BR2','ORG61BR1','ORG65BR1','ORG65BR2', 'ORG66BR2','ORG66BR3','ORG73BR1','ORG76BR1')

### Create empty list to append data frames toA
df_list <- list()

### Iterate through list of defined organoids
for (i in 1:length(small_mixed_dense_organoids)) {
  organoid <- small_mixed_dense_organoids[i]
  ### Generate filepath for each organoid in list
  filepath <- paste0('output/', organoid, '/all_data_with_bins.csv')
  print(paste('Reading file:', filepath))
  ### Read cleaned DF and generate new combined ID column
  df <- read_csv(here::here(filepath), show_col_types = FALSE) %>%
    mutate(WELL_ID = paste0(Plate_ID, '_', WELL_ID))
  ### add dataframe to list
  df_list[[organoid]] <- df
  print(paste('CellProfiler data for', organoid, 'stored.'))
  print(paste("ncol=", ncol(df)))
  
}

### Combine all dataframes together
all_data_with_bins <- do.call(rbind, df_list)
all_data_with_bins <- all_data_with_bins[,-c(1)]
```
...
### Compare spheroid size between positive and negative controls 
...
### Compare spatial intensity distribution between positive and negative cointrols
...
### Compare viability (log2(CTG) )across positive and negative controls 
...
### remove plot variables from environment to clear up space
rm(plot,plot2, plot3)
```



Next, we generate an overlayed histogram of spheroid area distribtion to compare positive and negative controls and mark the respective median values. As expected, median spheroid area of negative controls (cells that are "likely alive" in Media/DMSO) is larger than positive controls.
...
#write.csv(all_data_clean, here::here(paste0("output/",patient,"/all_data_clean.csv")))

#write.csv(all_data_clean, here::here(paste0("output/",patient,"/Feature_reduced_raw_data.csv")))
```
...
### long format data = better for visualisation purposes
...
```{r}
### convert back to dataframa
dat2 <- control_short %>% as.data.frame() %>% 
  dplyr::select(-c(2:11))

### convert bin to a factor - needed for categorical classification models
dat2[, 1] <- as.factor(dat2[, 1])  

### Number of observations
n <- nrow(dat2)  

### number of observations needed to constitute 80% of observations for training set
ntrain <- round(n*0.80)  

### Set seed for reproducible results
set.seed(123)   

### Take random sample of indexes of size ntrain from n
tindex <- sample(n, ntrain)  

### Create training set using indexes from full dataset
train_dat2 <- dat2[tindex,]   

### Create test set of remaining 20% of idexes not used in training set
test_dat2 <- dat2[-tindex,]  
...
```{r}
### Training SVM to classify based on "Bin" factor variable (positive control VS negative control), based on all OTHER features 

svmfit2 = svm(Bin ~ ., 
              data = trainTransformed, 
              kernel="radial", # kernel = radial for nonlinear relationships
              probability=TRUE) # model will generate probability estimates based on probability = TRUE
...
### smaller gamma = broader influence, larger gamma = tighter influence
...
```{r}
### Test predictions on training set data
...
cf <- confusionMatrix(pred,trainTransformed$Bin)
cf
...
```{r}
### Randomly scramble bin (control) labels of training data set
trainTransformed_scr <- transform(trainTransformed, Bin = sample(Bin) )
```


By randomising/scrambling the category labels, we create a data set that should not be predicted correctly and indeed, the prediction accuracy for the scrambled data drops down to 52%. 
 
```{r}
pred_scr <- predict(svmfit2,trainTransformed_scr, probability = TRUE) 

### Evaluate performance 
cf <-confusionMatrix(pred_scr,trainTransformed_scr$Bin)
cf
fourfoldplot(as.table(cf),color=c("yellow","pink"),main = "Confusion Matrix")
```
...
**Here, changing cost from 1 to 2 and changing gamma from a default of 0.01 to 0.5 improved the model performance from 89% to 100% accuracy.**
...
obj <- tune(svm,
            ...
            data = trainTransformed, 
            kernel = "radial",
            ranges = list(gamma = c((1/num_features),(3/num_features),(5/num_features)), cost = c(1,2,3,4,5))
            )
```
...
save(svm_model_after_tune, file = here::here(paste0("output/",patient,"/svm_model_after_tune.RData")))

```

```{r}

load(here::here(paste0("output/",patient,"/svm_model_after_tune.RData")))
```
...
cf <- confusionMatrix(pred2,trainTransformed$Bin)
cf
fourfoldplot(as.table(cf),color=c("yellow","pink"),main = "Confusion Matrix")
```
...
plot2 <- trainTransformed_prob %>%
  ...
  theme(axis.text.x = element_text())+
  labs(title = "True labels vs Probabilities", y = "Probability of being categorised as alive", x="true labels") + 
...
plot2 <- trainTransformed_prob %>%
  ...
  theme(axis.text.x = element_text())+
  labs(title = "Scrambled labels vs Probabilities", y = "Probability of being categorised as alive", x="scrambled labels") + 
  geom_hline(aes(yintercept = 0.50), linetype="solid", color = "black")+
    theme(legend.title = element_text(), legend.text = element_blank(), 
      axis.text.x = element_text(angle = 25, colour = "black"),
      axis.title.y = element_text(margin = margin(t = 0, r = 30, b = 0, l = 30)), 
...
Testing the SVM model on the last 20% of data gives a 95% accuracy in prediction.  

```{r}
prediction <- predict(svm_model_after_tune, testTransformed, probability = TRUE)

cf <- confusionMatrix(prediction,testTransformed$Bin)
cf
fourfoldplot(as.table(cf),color=c("yellow","pink"),main = "Confusion Matrix")
```
...
Testing the SVM model on the last 20% of data after scrambling the category labels gives a random 54% accuracy in prediction.  
...
cf <-confusionMatrix(prediction_scr,test_dat2_scr$Bin)
cf
fourfoldplot(as.table(cf),color=c("yellow","pink"),main = "Confusion Matrix")
```
...
#write.csv(data_all_short_all, "data_all_short_all1.csv")
...
#write.csv(summary, here::here(paste0("output/",patient,"/summary_morphology.csv")))
```
...
#write.csv(pred_all_data, here::here(paste0("output/", patient,"/pred_all_data.csv")))
...
#write.csv(pred_all_data_mean, here::here(paste0("output/",patient,"/pred_all_data_mean_ORG66BR3.csv")))
```
...
normplot1 <- pred_all_data_mean %>%
  ...
  geom_text(data = r2_by_patient[1,],aes(label = sprintf("R^2 [%s] = %.2f",Plate_ID,r2),x = Inf, y = Inf), hjust = 1, vjust = 1, inherit.aes = FALSE, size = 5) +
  geom_text(data = r2_by_patient[2,],aes(label = sprintf("R^2 [%s] = %.2f",Plate_ID,r2),x = Inf, y = Inf), hjust = 1, vjust = 2.5, inherit.aes = FALSE, size = 5) +
  geom_text(data = r2_by_patient[3,],aes(label = sprintf("R^2 [%s] = %.2f",Plate_ID,r2),x = Inf, y = Inf), hjust = 1, vjust = 4, inherit.aes = FALSE, size = 5) +
...
### For individual organoids {.tabset}


```{r}
ctg_plot_list = list()
for (organoid in small_mixed_dense_organoids){
  ### Subset df to only include org of interest
  data <- pred_all_data_mean[pred_all_data_mean$Plate_ID==organoid,]
  r2_data <- r2_by_patient[r2_by_patient$Plate_ID==organoid,]
  #print(r2_data)
  #print(r2_data[1,])
  ...
  geom_point(aes(colour = Patient,
                 ...
                 alpha = 0.8, size = 1.6) +
  geom_smooth(method = "lm", se = FALSE, color = "black", linetype = "dashed") +
  geom_text(data = r2_data,aes(label = sprintf("R^2 [%s] = %.2f",Plate_ID,r2),x = Inf, y = Inf), hjust = 1, vjust = 1, inherit.aes = FALSE, size = 5) +
  labs(title = "CTG Norm vs SVM probabilities \n", x = "CTG Raw", y = "Probability of being alive") +
  theme_bw() +
  theme(axis.title.x = element_text(margin = margin(t = 10, r = 10, b = 20, l = 20)), 
        axis.title.y = element_text(margin = margin(t = 0, r = 10, b = 0, l = 20)),
  ...
  ctg_plot_list[[organoid]] <- normplot1
}
```
...
ctg_plot_list[['ORG60BR3']]
```

#### ORG64BR3

```{r, fig.height=4, fig.width=6}
ctg_plot_list[['ORG64BR3']]
```

#### ORG70BR1

```{r, fig.height=4, fig.width=6}
ctg_plot_list[['ORG70BR1']]
```


## SVM-probabilities dose-response curves compared to CTG {.tabset}
...
for (organoid in small_mixed_dense_organoids){
  ...
  normplot <- data_norm_plot %>%
    ...
    labs(title = paste("SVM results", organoid, "\n"),
