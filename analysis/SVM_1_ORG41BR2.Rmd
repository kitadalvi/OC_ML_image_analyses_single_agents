---
title: "<img src=\"Cropped_VCFG_Logo2019.png\" style=\"float:right;height:77.1429px;width:152px\"/>ORG41BR2<br>3D Image-based Classification <br> SVM analyses<br>"
output: 
  html_document:
    keep_md: true
    theme: flatly
...
* Alive/ healthy control (Vehicle control Wells from patient ORG41BR2)  
...
annotationsCTG <- read_csv(here::here("data/OC_ORG41BR2_CTG.csv")) %>% dplyr::select(-WELL_ID)
```

Normalise CTG to media and DMSO controls.

```{r}
annotationsCTG <- read_csv(here::here("data/OC_ORG41BR2_CTG.csv")) 
...
**Here, changing cost from 1 to 2 and keep gamma at default improved the model performance from 93% to 96% accuracy.**
...
Testing the SVM model on the last 20% of data after scrambling the category labels gives a random 51% accuracy in prediction.  
...
write.csv(summary, here::here(paste0("output/",patient,"/summary_morphology_ORG41BR2.csv")))
```
...
write.csv(pred_all_data, here::here(paste0("output/", patient,"/pred_all_data_ORG41BR2.csv")))
...
write.csv(pred_all_data_mean, here::here(paste0("output/",patient,"/pred_all_data_mean_ORG41BR2.csv")))
```
...
normplot <- data_norm_plot %>%
  ...
  labs(title = "SVM results ORG41BR2\n",
