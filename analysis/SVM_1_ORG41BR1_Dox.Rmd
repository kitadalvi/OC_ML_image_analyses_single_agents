---
title: "<img src=\"Cropped_VCFG_Logo2019.png\" style=\"float:right;height:77.1429px;width:152px\"/>ORG41BR1<br>3D Image-based Classification <br> SVM analyses<br>"
output: 
  html_document:
    keep_md: true
    theme: flatly
...
* Alive/ healthy control (Vehicle control Wells from patient ORG41BR1)  
...
The following Wells were excluded from Media and DMSO controls: B13, H13, M2. <br>
...
annotationsCTG <- read_csv(here::here("data/OC_ORG41BR1_CTG.csv")) %>% dplyr::select(-WELL_ID)
```

Normalise CTG to media and DMSO controls.

```{r}
annotationsCTG <- read_csv(here::here("data/OC_ORG41BR1_CTG.csv")) 
...
By randomising/scrambling the category labels, we create a data set that should not be predicted correctly and indeed, the prediction accuracy for the scrambled data drops down to 56%. 
...
**Here, changing cost from 1 to 2 and keep gamma at default improved the model performance from 97% to 98% accuracy.**
...
Testing the SVM model on the last 20% of data gives a 96% accuracy in prediction.  
...
write.csv(summary, here::here(paste0("output/",patient,"/summary_morphology_ORG41BR1.csv")))
```
...
write.csv(pred_all_data, here::here(paste0("output/", patient,"/pred_all_data_ORG41BR1.csv")))
...
write.csv(pred_all_data_mean, here::here(paste0("output/",patient,"/pred_all_data_mean_ORG41BR1.csv")))
```
...
normplot <- data_norm_plot %>%
  ...
  labs(title = "SVM results ORG41BR1\n",
...
plot(
  ...
  testTransformed,
  Texture_InverseDifferenceMoment_CropDAPI_3_01_256 ~ Texture_DifferenceVariance_CropDAPI_3_01_256,
...
x_var <- "Texture_InverseDifferenceMoment_CropDAPI_3_01_256"
y_var <- "Texture_DifferenceVariance_CropDAPI_3_01_256"
