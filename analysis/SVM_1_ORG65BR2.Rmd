---
title: "<img src=\"Cropped_VCFG_Logo2019.png\" style=\"float:right;height:77.1429px;width:152px\"/>ORG65BR2<br>3D Image-based Classification <br> SVM analyses<br>"
output: 
  html_document:
    keep_md: true
    theme: flatly
...
* Alive/ healthy control (Vehicle control Wells from patient ORG65BR2)  
...
The following Wells were excluded from Media and DMSO controls: H13-23, O13-23, N13-15, G13-15, I13-15, D14, K14, F3, M3. <br>

*in addition to Staurosporine and high-dose Mitomycin, we added high-dose Doxorubicin and high-dose Topotecan to the positive controls.*
...
annotationsCTG <- read_csv(here::here("data/OC_ORG65BR2_CTG.csv")) %>% dplyr::select(-WELL_ID)
```

Normalise CTG to media and DMSO controls.

```{r}
annotationsCTG <- read_csv(here::here("data/OC_ORG65BR2_CTG.csv")) 

# calculate normalised values
norm_CTG <- annotationsCTG %>%
  filter(Compound_ID %in% c("Media", "DMSO")) %>% 
  ### list excluded wells below. 
  filter(!Well_ID %in% c('H13','H14','H15','H16','H17','H18','H19','H20','H21','H22','H23','O13','O14','O15','O16','O17','O18','O19','O20','O21','O22','O23','N13','N14','N15','G13','G14','G15','I13','I14','I15','D14','K14','F3','M3')) %>% 
  group_by(Patient) %>% 
...
write.csv(all_data_with_bins, here::here(paste0("output/",patient,"/all_data_with_bins.csv")))
...
Next, we generate an overlayed histogram of spheroid area distribtion to compare positive and negative controls and mark the respective median values. As expected, median spheroid area of negative controls (cells that are "likely alive" in Media/DMSO) is larger than positive controls (cells that are "likely dead" in Straurosporine/ Doxorubicin/ Mitomycin C/ Topotecan).
...
write.csv(CorData, here::here(paste0("output/",patient,"/CorData_removed.csv")))
...
By randomising/scrambling the category labels, we create a data set that should not be predicted correctly and indeed, the prediction accuracy for the scrambled data drops down to 57%. 
...
**Here, changing cost from 1 to 5 and changing gamma from 0.01 to 0.02 improved the model performance from 98% to 99% accuracy.**
...
Testing the SVM model on the last 20% of data gives a 97% accuracy in prediction.  
...
write.csv(summary, here::here(paste0("output/",patient,"/summary_morphology_ORG65BR2.csv")))
```
...
write.csv(pred_all_data, here::here(paste0("output/", patient,"/pred_all_data_ORG65BR2.csv")))
...
write.csv(pred_all_data_mean, here::here(paste0("output/",patient,"/pred_all_data_mean_ORG65BR2.csv")))
```
...
normplot <- data_norm_plot %>%
  ...
  labs(title = "SVM results ORG65BR2\n",
...
# assume you have trained:
# svm_model <- svm(Bin ~ ., data = df, type = "C-classification", kernel = "radial")

df <- trainTransformed
# pick two numeric predictors to visualise
x_var <- "Texture_DifferenceVariance_CropDAPI_3_03_256"
y_var <- "Texture_InfoMeas2_CropDAPI_3_02_256"

# create sequences over the ranges
x_seq <- seq(min(df[[x_var]])-1, max(df[[x_var]])+1, by = 0.01)
y_seq <- seq(min(df[[y_var]])-1, max(df[[y_var]])+1, by = 0.01)
...
# For all other predictors, set to their median (or a typical value)
for (col in setdiff(names(df), c("Bin", x_var, y_var))) {
  if (is.numeric(df[[col]])) {
    grid[[col]] <- median(df[[col]], na.rm = TRUE)
  } else {
    # if any factor, choose first level
    grid[[col]] <- levels(df[[col]])[1]
  }
}

# predict class for each grid point
grid$pred <- predict(svm_model_after_tune, newdata = grid, probability = TRUE)

# convert pred to factor if needed
grid$pred <- factor(grid$pred, levels = levels(df$Bin))

# plot
ggplot() +
  geom_tile(data = grid,
            aes_string(x = x_var, y = y_var, fill = "pred"),
            alpha = 0.3) +
  geom_point(data = df,
             aes_string(x = x_var, y = y_var, color = "Bin"),
             size = 1.5, alpha = 0.8) +
  scale_fill_manual(values = c("negative_control" = "lightpink", 
                               "positive_control" = "lightgreen"),
                    name = "Predicted class") +
  scale_color_manual(values = c("negative_control" = "red", 
                                "positive_control" = "darkgreen"),
                     name = "Actual Bin") +
  labs(title = "SVM Decision Boundary for Bin",
       x = x_var,
       y = y_var) +
  theme_minimal()
