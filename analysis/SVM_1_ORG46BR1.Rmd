---
title: "<img src=\"Cropped_VCFG_Logo2019.png\" style=\"float:right;height:77.1429px;width:152px\"/>ORG46BR1<br>3D Image-based Classification <br> SVM analyses<br>"
output: 
  html_document:
    keep_md: true
    theme: flatly
...
* Alive/ healthy control (Vehicle control Wells from patient ORG46BR1)  
...
annotationsCTG <- read_csv(here::here("data/OC_ORG46BR1_CTG.csv")) %>% dplyr::select(-WELL_ID)
```

Normalise CTG to media and DMSO controls.

```{r}
annotationsCTG <- read_csv(here::here("data/OC_ORG46BR1_CTG.csv")) 
...
**Here, changing cost from 1 to 8 and changing gamma from 0.01 to 0.05 improved the model performance from 87% to 98% accuracy.**
...
Testing the SVM model on the last 20% of data gives a 82% accuracy in prediction.  
...
write.csv(summary, here::here(paste0("output/",patient,"/summary_morphology_ORG46BR1.csv")))
```
...
write.csv(pred_all_data, here::here(paste0("output/", patient,"/pred_all_data_ORG46BR1.csv")))
...
write.csv(pred_all_data_mean, here::here(paste0("output/",patient,"/pred_all_data_mean_ORG46BR1.csv")))
```
...
normplot <- data_norm_plot %>%
  ...
  labs(title = "SVM results ORG46BR1\n",
