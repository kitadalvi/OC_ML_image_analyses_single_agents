---
title: "<img src=\"Cropped_VCFG_Logo2019.png\" style=\"float:right;height:77.1429px;width:152px\"/>ORG70BR1<br>3D Image-based Classification <br> SVM analyses<br>"
output: 
  html_document:
    keep_md: true
    theme: flatly
...
* Alive/ healthy control (Vehicle control Wells from patient ORG70BR1)  
...
The following Wells were excluded from Media and DMSO controls: H13-23, O13-23, N13-15, G13-15, I13-15, D14, K14, F3, M3, H2, O12. <br>
...
annotationsCTG <- read_csv(here::here("data/OC_ORG70BR1_CTG.csv")) %>% dplyr::select(-WELL_ID)
...
```{r}
annotationsCTG <- read_csv(here::here("data/OC_ORG70BR1_CTG.csv")) 

# calculate normalised values
norm_CTG <- annotationsCTG %>%
  filter(Compound_ID %in% c("Media", "DMSO")) %>% 
  ### list exluded wells below. 
  filter(!Well_ID %in% c('H13','H14','H15','H16','H17','H18','H19','H20','H21','H22','H23','O13','O14','O15','O16','O17','O18','O19','O20','O21','O22','O23','N13','N14','N15','G13','G14','G15','I13','I14','I15','D14','K14','F3','M3', 'O12')) %>% 
  group_by(Patient) %>% 
...
*Need to update with images for ORG70BR1*
```{r, echo=FALSE}
...
**Here, changing cost from 1 to 10 and changing gamma from 0.012 to 0.015 improved the model performance from 98% to 99% accuracy.**
...
Testing the SVM model on the last 20% of data after scrambling the category labels gives a random 52% accuracy in prediction.  
...
write.csv(summary, here::here(paste0("output/",patient,"/summary_morphology_ORG70BR1.csv")))
```
...
write.csv(pred_all_data, here::here(paste0("output/", patient,"/pred_all_data_ORG70BR1.csv")))
...
write.csv(pred_all_data_mean, here::here(paste0("output/",patient,"/pred_all_data_mean_ORG70BR1.csv")))
```
...
normplot <- data_norm_plot %>%
  ...
  labs(title = "SVM results ORG70BR1\n",
