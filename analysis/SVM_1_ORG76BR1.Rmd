---
title: "<img src=\"Cropped_VCFG_Logo2019.png\" style=\"float:right;height:77.1429px;width:152px\"/>ORG76BR1<br>3D Image-based Classification <br> SVM analyses<br>"
output: 
  html_document:
    keep_md: true
    theme: flatly
...
* Alive/ healthy control (Vehicle control Wells from patient ORG76BR1)  
...
annotationsCTG <- read_csv(here::here("data/OC_ORG76BR1_CTG.csv")) %>% dplyr::select(-WELL_ID)
```

Normalise CTG to media and DMSO controls.

```{r}
annotationsCTG <- read_csv(here::here("data/OC_ORG76BR1_CTG.csv")) 
...
By randomising/scrambling the category labels, we create a data set that should not be predicted correctly and indeed, the prediction accuracy for the scrambled data drops down to 54%. 
...
**Here, changing cost from 1 to 10 and changing gamma to 0.05 improved the model performance from 92% to 99% accuracy.**
...
Testing the SVM model on the last 20% of data gives a 85% accuracy in prediction.  
...
write.csv(summary, here::here(paste0("output/",patient,"/summary_morphology_ORG76BR1.csv")))
```
...
write.csv(pred_all_data, here::here(paste0("output/", patient,"/pred_all_data_ORG76BR1.csv")))
...
write.csv(pred_all_data_mean, here::here(paste0("output/",patient,"/pred_all_data_mean_ORG76BR1.csv")))
```
...
normplot <- data_norm_plot %>%
  ...
  labs(title = "SVM results ORG76BR1\n",
