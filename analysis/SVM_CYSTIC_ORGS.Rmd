---
title: "<img src=\"Cropped_VCFG_Logo2019.png\" style=\"float:right;height:77.1429px;width:152px\"/>CYSTIC ORGANOIDS<br>3D Image-based Classification <br> SVM analyses<br>"
output: 
  html_document:
    keep_md: true
    theme: flatly
...
* Alive/ healthy control (Vehicle control Wells from CYSTIC organoids)  
...
for (i in 1:length(cystic_organoids)) {
  ...
  df <- read_csv(here::here(filepath)) %>%
...
cf
fourfoldplot(as.table(cf),color=c("yellow","pink"),main = "Confusion Matrix before tuning")
```
...
By randomising/scrambling the category labels, we create a data set that should not be predicted correctly and indeed, the prediction accuracy for the scrambled data drops down to 55%. 
...
**Here, changing cost from 1 to 2 and changing gamma from a default of 0.01 to 0.03 improved the model performance from 97% to 98% accuracy.**
...
cf
fourfoldplot(as.table(cf),color=c("yellow","pink"),main = "Confusion Matrix after tuning")
```
...
```{r}
ctg_plot_list = list()
for (organoid in cystic_organoids){
