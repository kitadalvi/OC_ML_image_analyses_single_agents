---
title: "<img src=\"Cropped_VCFG_Logo2019.png\" style=\"float:right;height:77.1429px;width:152px\"/>ORG61BR1<br>3D Image-based Classification <br> SVM analyses<br>"
output: 
  html_document:
    keep_md: true
    theme: flatly
...
* Alive/ healthy control (Vehicle control Wells from patient ORG61BR1)  
...
annotationsCTG <- read_csv(here::here("data/OC_ORG61BR1_CTG.csv")) %>% dplyr::select(-WELL_ID)
...
```{r}
annotationsCTG <- read_csv(here::here("data/OC_ORG61BR1_CTG.csv")) 
...
By randomising/scrambling the category labels, we create a data set that should not be predicted correctly and indeed, the prediction accuracy for the scrambled data drops down to 65%. 
...
**Here, changing cost from 1 to 15 and increasing gamma from 0.01 to 0.03 improved the model performance from 98% to 100% accuracy.**
...
Testing the SVM model on the last 20% of data gives a 88% accuracy in prediction.  
...
Testing the SVM model on the last 20% of data after scrambling the category labels gives a random 66% accuracy in prediction.  
...
write.csv(summary, here::here(paste0("output/",patient,"/summary_morphology_ORG61BR1.csv")))
```
...
write.csv(pred_all_data, here::here(paste0("output/", patient,"/pred_all_data_ORG61BR1.csv")))
...
write.csv(pred_all_data_mean, here::here(paste0("output/",patient,"/pred_all_data_mean_ORG61BR1.csv")))
```
...
normplot <- data_norm_plot %>%
  ...
  labs(title = "SVM results ORG61BR1\n",
