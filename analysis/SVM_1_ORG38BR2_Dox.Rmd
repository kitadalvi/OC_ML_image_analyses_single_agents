---
title: "<img src=\"Cropped_VCFG_Logo2019.png\" style=\"float:right;height:77.1429px;width:152px\"/>ORG38BR2<br>3D Image-based Classification <br> SVM analyses<br>"
output: 
  html_document:
    keep_md: true
    theme: flatly
...
annotationsCTG <- read_csv(here::here("data/OC_ORG38BR2_CTG.csv")) %>% dplyr::select(-WELL_ID)
...
Images of negative control wells excluded from analysis: M2

```{r, echo=FALSE}
htmltools::img(src = knitr::image_uri(here::here("data/images", "ORG38BR2_M2.png")), 
...
```{r}
annotationsCTG <- read_csv(here::here("data/OC_ORG38BR2_CTG.csv")) 
...
*Need to update with images for ORG38BR2*
```{r, echo=FALSE}
...
By randomising/scrambling the category labels, we create a data set that should not be predicted correctly and indeed, the prediction accuracy for the scrambled data drops down to 49%. 
...
**Here, changing cost from 1 to 25 and keep gamma at default improved the model performance from 95% to 99% accuracy.**
...
Testing the SVM model on the last 20% of data gives a 93% accuracy in prediction.  
...
#write.csv(summary, here::here(paste0("output/",patient,"/summary_morphology_ORG38BR2.csv")))
```
...
#write.csv(pred_all_data, here::here(paste0("output/", patient,"/pred_all_data_ORG38BR2.csv")))
...
#write.csv(pred_all_data_mean, here::here(paste0("output/",patient,"/pred_all_data_mean_ORG38BR2.csv")))
```
...
normplot <- data_norm_plot %>%
  ...
  labs(title = "SVM results ORG38BR2\n",
