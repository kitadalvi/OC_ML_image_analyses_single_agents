...
norm_CTG <- annotationsCTG %>%
  ...
  filter(!Well_ID %in% c('B13','H13','M2')) %>% 
  group_by(Patient) %>% 
...
By randomising/scrambling the category labels, we create a data set that should not be predicted correctly and indeed, the prediction accuracy for the scrambled data drops down to 50%. 
...
Testing the SVM model on the last 20% of data gives a 92% accuracy in prediction.  
