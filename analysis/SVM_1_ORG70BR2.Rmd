---
title: "<img src=\"Cropped_VCFG_Logo2019.png\" style=\"float:right;height:77.1429px;width:152px\"/>ORG70BR2<br>3D Image-based Classification <br> SVM analyses<br>"
output: 
  html_document:
    keep_md: true
    theme: flatly
...
* Alive/ healthy control (Vehicle control Wells from patient ORG70BR2)  
...
The following Wells were excluded from Media and DMSO controls: H13-23, O13-23, N13-15, G13-15, I13-15, D14, K14, F3, M3, I4. <br>
...
annotationsCTG <- read_csv(here::here("data/OC_ORG70BR2_CTG.csv")) %>% dplyr::select(-WELL_ID)
...
```{r}
annotationsCTG <- read_csv(here::here("data/OC_ORG70BR2_CTG.csv")) 

# calculate normalised values
norm_CTG <- annotationsCTG %>%
  filter(Compound_ID %in% c("Media", "DMSO")) %>% 
  ### list exluded wells below. 
  filter(!Well_ID %in% c('H13','H14','H15','H16','H17','H18','H19','H20','H21','H22','H23','O13','O14','O15','O16','O17','O18','O19','O20','O21','O22','O23','N13','N14','N15','G13','G14','G15','I13','I14','I15','D14','K14','F3','M3', 'I4')) %>% 
  group_by(Patient) %>% 
...
*Need to update with images for ORG70BR2*
```{r, echo=FALSE}
...
cf
fourfoldplot(as.table(cf),color=c("red3","skyblue2"),main = "Confusion Matrix")
```
...
By randomising/scrambling the category labels, we create a data set that should not be predicted correctly and indeed, the prediction accuracy for the scrambled data drops down to 51%. 
...
**Here, changing cost from 1 to 3 and keeping default gamma improved the model performance from 98% to 99% accuracy.**
...
Testing the SVM model on the last 20% of data after scrambling the category labels gives a random 49% accuracy in prediction.  
...
#write.csv(summary, here::here(paste0("output/",patient,"/summary_morphology_ORG70BR2.csv")))
```
...
#write.csv(pred_all_data, here::here(paste0("output/", patient,"/pred_all_data_ORG70BR2.csv")))
...
#write.csv(pred_all_data_mean, here::here(paste0("output/",patient,"/pred_all_data_mean_ORG70BR2.csv")))
```
...
normplot <- data_norm_plot %>%
  ...
  labs(title = "SVM results ORG70BR2\n",
...
```{r, fig.height=10, fig.width=10}
plot_data <- data_norm_plot[data_norm_plot$variable == 'CTG' | data_norm_plot$variable == 'prob_alive',]
normplot <- plot_data %>%
  unite("Identifyer", c(Plate_ID, Patient), sep = "_", remove = FALSE) %>% 
  ggplot(., aes(x = Concentration, 
                y = mean,
                group = variable,
                colour = variable)) +
  facet_wrap(~Compound_ID, ncol = 3, scales = 'fixed')  +
  scale_x_continuous(trans = "log10", labels = scales::number_format(accuracy = 0.1)) + 
  geom_smooth(method = 'loess') +
  scale_colour_manual(values = c("CTG"="red", "prob_alive"="blue")) +
  labs(title = "SVM results ORG70BR2\n",
       x = "Log10 Concentration (uM)",
       y = "Probability of being alive") +
  theme_light()+
