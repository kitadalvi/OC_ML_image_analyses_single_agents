---
title: "<img src=\"Cropped_VCFG_Logo2019.png\" style=\"float:right;height:77.1429px;width:152px\"/>ORG73BR1<br>3D Image-based Classification <br> SVM analyses<br>"
output: 
  html_document:
    keep_md: true
    theme: flatly
...
* Alive/ healthy control (Vehicle control Wells from patient ORG73BR1)  
...
The following Wells were excluded from Media and DMSO controls: H13-23, O13-23, N13-15, G13-15, I13-15, D14, K14, F3, M3, O2. <br>
...
annotationsCTG <- read_csv(here::here("data/OC_ORG73BR1_CTG.csv")) %>% dplyr::select(-WELL_ID)

#generate_heatmaps(annotationsCTG)

# annotationsCTG2 <- read_csv(here::here("data", "OC_ORG66BR3_CTG.csv")) %>% 
#    left_join(annotationsCTG, by = c("Well_ID", "Concentration", "Units", "Description", "Compound_ID", "Patient")) 
...
```{r}
annotationsCTG <- read_csv(here::here("data/OC_ORG73BR1_CTG.csv")) 

# calculate normalised values
norm_CTG <- annotationsCTG %>%
  filter(Compound_ID %in% c("Media", "DMSO")) %>% 
  ### list exluded wells below. 
  filter(!Well_ID %in% c('H13','H14','H15','H16','H17','H18','H19','H20','H21','H22','H23','O13','O14','O15','O16','O17','O18','O19','O20','O21','O22','O23','N13','N14','N15','G13','G14','G15','I13','I14','I15','D14','K14','F3','M3', 'O2')) %>% 
  group_by(Patient) %>% 
...
### Compare spatial intensity distribution between positive and negative controls
...
*Need to update with images for ORG73BR1*
```{r, echo=FALSE}
...
By randomising/scrambling the category labels, we create a data set that should not be predicted correctly and indeed, the prediction accuracy for the scrambled data drops down to 64%. 
...
**Here, changing cost from 1 to 3 and changing gamme from 0.01 to 0.02 improved the model performance from 98% to 99% accuracy.**
...
conf_matrix <- table(trainTransformed$Bin, pred2)
...
Testing the SVM model on the last 20% of data gives a 99% accuracy in prediction.  
...
Testing the SVM model on the last 20% of data after scrambling the category labels gives a random 64% accuracy in prediction.  
...
write.csv(summary, here::here(paste0("output/",patient,"/summary_morphology_ORG73BR1.csv")))
```
...
write.csv(pred_all_data, here::here(paste0("output/", patient,"/pred_all_data_ORG73BR1.csv")))
...
write.csv(pred_all_data_mean, here::here(paste0("output/",patient,"/pred_all_data_mean_ORG73BR1.csv")))
```
...
normplot <- data_norm_plot %>%
  ...
  labs(title = "SVM results ORG73BR1\n",
...
```{r}
### Tetsing AUC dose response curves 
pred_data <- read.csv('../output/ORG73BR1/pred_all_data.csv')
colnames(pred_data)
df <- pred_data[,c(5,6,7,8,9,10,12,3)] 
df$conddition <- paste0(df$Compound_ID,'_', df$Concentration)
datatable(df)

compounds <- levels(as.factor(df$Compound_ID))

for (compound in compounds) {
  plot_data <- df[df$Compound_ID == compound,]
  plot <- ggplot(data=plot_data, aes(x=Concentration, y=negative_control)) +
    geom_line()+
    geom_point() +
    ggtitle(`compound`)
  print(plot)
...
```{r}
### Tetsing AUC dose response curves 
df <- read.csv('../output/ORG70BR2/pred_all_data_mean_ORG70BR2.csv')
colnames(df)
df$condition <- paste0(df$Compound_ID,'_', df$Concentration) 
df$log_conc <- log10(as.numeric(df$Concentration))
datatable(df)
...
plot <- ggplot(data=df, aes(x=log_conc, y=prob_alive)) +
  facet_wrap(~Compound_ID, scales = "fixed") +
...
### Tetsing AUC dose response curves 
df <- read.csv('../output/ORG70BR2/pred_all_data_ORG70BR2.csv')
colnames(df)
df$condition <- paste0(df$Compound_ID,'_', df$Concentration) 
df$log_conc <- log10(as.numeric(df$Concentration))
datatable(df)
...
plot <- ggplot(data=df, aes(x=log_conc, y=negative_control)) +
  facet_wrap(~Compound_ID, scales = "fixed") +
  geom_smooth(method = 'loess') +
  geom_point() 
