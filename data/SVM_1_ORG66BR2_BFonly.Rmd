---
title: "<img src=\"Cropped_VCFG_Logo2019.png\" style=\"float:right;height:77.1429px;width:152px\"/>ORG66BR2<br>3D Image-based Classification <br> SVM analyses<br> Use Brightfield only"
output: 
  html_document:
    keep_md: true
    theme: flatly
...
The readouts from the high-content image analysis include morphology, intensity, and texture features of the spheroids in Brightfield images.
...
The following Wells were excluded from Media and DMSO controls: H13, G13, I13, N13, M13, F13, F12, G12, L13, C13, F2, L2
...
annotationsCTG <- read_csv(here::here("data/OC_ORG66BR2_annotation.csv")) %>% dplyr::select(-WELL_ID)

generate_heatmaps(annotationsCTG)

# annotationsCTG <- read_csv(here::here("data", "OC_ORG66BR2_annotation.csv")) %>% 
...
#write.csv(all_data_with_bins, here::here("output/all_data_with_bins_BF_only.csv"))
...
for (column_name in column_names[13:length(column_names)]) {
  ...
  if (check_column(column) || is.character(column) || all(column %in% c(0, -1)) || 
      any(grepl("Execution|Align|Height|Group_|ImageNumber|EulerNumber|_Center_|Scaling|Threshold|Width|Channel|Location|CropDAPI", column_name))) {
...
all_data_clean <- all_data_with_bins %>% 
  ...
  filter(!AreaShape_Area<500) 

clean_features <- names(all_data_clean[,-c(1:13)])
#write.csv(all_data_clean, here::here("output/all_data_clean_BF_only.csv"))

#write.csv(all_data_clean, here::here("output/Feature_reduced_raw_data_BF_only.csv"))
```
...
#write.csv(data_all_short, "data_all_short_BF_only.csv")
...
CleanedAn_melt <- raw2 %>% 
  melt(id.vars = id, measure.vars = features) 
...
**Here, changing cost from 1 to 50 and keep gamma at default improved the model performance from 92% to 97% accuracy.**
...
save(svm_model_after_tune, file = here::here("output/svm_model_after_tune_BF_only.RData"))

```
```{r}

load(here::here("output/svm_model_after_tune_BF_only.RData"))
```
...
Testing the SVM model on the last 20% of data after scrambling the category labels gives a random 50% accuracy in prediction.  
...
#write.csv(data_all_short_all, "data_all_short_all1_BF_only.csv")
...
write.csv(summary, here::here("output/summary_morphology_ORG66BR2_BF_only.csv"))
```
...
pred_all_data_mean<- pred_all_data %>% ungroup() %>% 
  dplyr::select(Plate_ID, Replicate, Patient, Description, Compound_ID, Concentration,  WELL_ID, CTG, Bin, negative_control, positive_control) %>%
  group_by(Plate_ID, WELL_ID, Patient, Description, Compound_ID, Concentration) %>%
 dplyr::mutate(prob_alive = mean(negative_control), prob_dead = mean(positive_control)) %>%
  dplyr::select(-c(positive_control, negative_control))%>%
  unique() %>%
  ungroup()

write.csv(pred_all_data, here::here("output/pred_all_data_ORG66BR2_BF_only.csv"))
...
#write.csv(pred_all_data, here::here("output/pred_all_data_BF_only.csv"))
```
...
write.csv(pred_all_data_mean, here::here("output/pred_all_data_mean_ORG66BR2_BF_only.csv"))
```
...
### Probability of classified as alive
...
for (i in plate_list){
  ...
  plots <- data_plot %>%
    dplyr::group_by(Batch) %>%
    dplyr::do(plots = platetools::raw_map(data = .$CTG,
                                          well = .$WELL_ID,
                                          plate = 384) + 
                ggtitle(.$Batch[1]) +
                scale_fill_gradient2(low = "blue", mid = "white",
...
```{r, fig.height=10, fig.width=10}
# avoid the use of scientific notions
options(scipen=999)
